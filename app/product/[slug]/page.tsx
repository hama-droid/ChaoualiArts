...
export default function ProductPage({ params }: { params: { slug: string } }) {
  ...
  const mailSubject = encodeURIComponent(`Original Inquiry — ${product.title}`);
  const mailBody = encodeURIComponent(
    `Hello,\n\nI’m interested in the original artwork:\n\nTitle: ${product.title}\nSlug: ${product.slug}\n\nPlease share:\n- Availability\n- Price\n- Shipping to my country\n- Timeline\n\nMy name:\nMy location:\n\nThank you.`
  );
        ...
        <div className="grid gap-10 md:grid-cols-12">
          ...
          <aside className="md:col-span-5">
            ...
            <div className="mt-5 rounded-3xl border border-neutral-200 p-5">
              {isOriginal ? (
                <>
                  <p className="text-sm text-neutral-600">Collector purchase</p>
                  <p className="mt-2 text-sm text-neutral-700 leading-relaxed">
                    Originals are offered privately to collectors. Contact the studio to acquire this work.
                  </p>

                  <a
                    href={`mailto:${ORDER_EMAIL}?subject=${mailSubject}&body=${mailBody}`}
                ...
                <>
                  ...
                  <div className="mt-2 text-sm text-neutral-800 space-y-1">
                    <div>
                      USD: <b>${product.price.USD}</b>
                    </div>
                    <div>
                      EUR: <b>€{product.price.EUR}</b>
                    </div>
                    <div>
                      GBP: <b>£{product.price.GBP}</b>
                    </div>
                  </div>
